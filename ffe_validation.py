...
FFE_CSV = os.path.join(BASE_DIR, "ffe_depth_outputs", "ffe_results.csv")
IMAGE_ROOT = os.path.join(BASE_DIR, "GSV_Downloads_Walk1", "content", "GSV_Downloads_Walk")

if not os.path.exists(FFE_CSV):
    raise RuntimeError(f"ffe_results.csv not found: {FFE_CSV}")

print("Loading:", FFE_CSV)
ffe_df = pd.read_csv(FFE_CSV)
...
OUT_CSV = os.path.join(BASE_DIR, "ffe_validation_ground_relative.csv")
