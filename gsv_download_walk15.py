...
INPUT_CSV = os.path.join(PIPELINE_ROOT, "addresses.csv")
OUTPUT_ROOT = os.path.join(PIPELINE_ROOT, "gsv_images")
...
def download_for_one_house(house_row):

    h_lat = house_row["lat"]
    h_lng = house_row["lng"]
    full_address = str(house_row["address"])
...
def main():
    ...
    if not os.path.exists(INPUT_CSV):
        raise FileNotFoundError("addresses.csv not found")

    os.makedirs(OUTPUT_ROOT, exist_ok=True)

    df = pd.read_csv(INPUT_CSV)

    print(f"Downloading GSV for {len(df)} houses")

    for _, row in df.iterrows():
        download_for_one_house(row)

    print("Finished GSV download")
