...
#
# Features:
# 1) Reads input automatically from PIPELINE_ROOT/gsv_images
# 2) Writes outputs automatically under PIPELINE_ROOT
# 3) Auto-adds local detectron2 source directory to sys.path
# 4) Keeps original output naming to stay compatible with downstream scripts:
#    - best_views_top1_locked
#    - best_views_top1_locked_vis
#    - debug_all_locked
#    - debug_all_boxes_vis
#    - debug_all_boxes_json
#    - selection_locked_summary.csv
...
# =========================================================
# Resolve project / detectron2 source path BEFORE imports
# =========================================================
PROJECT_ROOT = os.path.dirname(os.path.abspath(__file__))

# Priority:
# 1) env DETECTRON2_SRC
# 2) project_root/detectron2-main
DETECTRON2_SRC = os.environ.get(
    "DETECTRON2_SRC",
    os.path.join(PROJECT_ROOT, "detectron2-main")
)

if os.path.isdir(DETECTRON2_SRC) and DETECTRON2_SRC not in sys.path:
    sys.path.insert(0, DETECTRON2_SRC)
...
try:
    ...
    raise RuntimeError(
        "Failed to import detectron2.\n"
        f"Current DETECTRON2_SRC = {DETECTRON2_SRC}\n"
        "Please make sure detectron2 source code exists there, or set env DETECTRON2_SRC."
    ) from e
...
print("DETECTRON2_SRC:", DETECTRON2_SRC)
print("====================================")
...
# Priority:
# 1) env DETECTRON_MODEL_PATH
# 2) fallback constant below (edit if you want)
MODEL_WEIGHTS_PATH = os.environ.get(
    "DETECTRON_MODEL_PATH",
    os.path.join(PROJECT_ROOT, "checkpoints", "model_final.pth")
)
...
# Keep OLD output names to stay compatible with downstream scripts
OUT_TOP1_VIS_DIR = os.path.join(PIPELINE_ROOT, "best_views_top1_locked_vis")
OUT_TOP1_DIR = os.path.join(PIPELINE_ROOT, "best_views_top1_locked")
OUT_DEBUG_ALL_DIR = os.path.join(PIPELINE_ROOT, "debug_all_locked")
OUT_CSV = os.path.join(PIPELINE_ROOT, "selection_locked_summary.csv")

OUT_ALL_BOX_VIS_DIR = os.path.join(PIPELINE_ROOT, "debug_all_boxes_vis")
OUT_ALL_BOX_JSON_DIR = os.path.join(PIPELINE_ROOT, "debug_all_boxes_json")
...
def x_target_from_meta(meta: dict, W: int):
    ...
    car_lat = float(meta["car_lat"])
    car_lng = float(meta["car_lng"])
    h_lat = float(meta["house_lat"])
    h_lng = float(meta["house_lng"])
...
def compute_rank_key_optionA_highest_conf_stairs(img_bgr, det_list_bound, in_fov_flag):
    ...
    info = {
        "reason": (
            f"entrance={hasEntrance} door={hasDoor} stairs={hasStairs} "
            f"goodStairs={hasGoodStairs} inFOV={bool(in_fov_flag)} "
            f"s_door={sD:.2f} s_stairs={sS:.2f} s_entr={s_entr:.2f} dist={dist_entr:.3f}"
        )
    }
...
def draw_vis(img_bgr, info, door_box, stairs_box, target_building_box, x_target, delta, in_fov, bind_mode, rank_key):
    ...
    rect(target_building_box, (0, 0, 255), 4)
    rect(door_box, (0, 255, 0), 3)
    rect(stairs_box, (0, 255, 255), 3)
    ...
    t3 = (
        f"x_target={None if x_target is None else round(x_target, 1)} "
        f"delta={None if delta is None else round(delta, 1)} "
        f"in_fov={in_fov} bind={bind_mode}"
    )
...
def main():
    if not os.path.isdir(GSV_ROOT):
        raise FileNotFoundError(f"GSV_ROOT not found: {GSV_ROOT}")
    ...
    subfolders = [f.path for f in os.scandir(GSV_ROOT) if f.is_dir()]
